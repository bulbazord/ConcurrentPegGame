...
----------------------------------------------------------------------------------------------------
-- Creates the initial state for each board.
----------------------------------------------------------------------------------------------------
makeInitialBoardState :: [Bool] -> Int -> Int -> [(Int, Int)] -> ([Bool], Int, Int, [(Int, Int)])
makeInitialBoardState board rows totalPegs moveList = (board, rows, totalPegs, moveList)

----------------------------------------------------------------------------------------------------
-- Solves a board based on its initial configuration
----------------------------------------------------------------------------------------------------
solveInitialBoard :: ([Bool], Int, Int, [(Int, Int)]) -> [(Int, Int)]
solveInitialBoard state = 
    do
        let (board, rows, totalPegs, moveList) = state
        recursiveSolve board rows totalPegs moveList
...
solve n =
    do
        ...
        let startStates = [ makeInitialBoardState (listOfBoards !! i) n numOfPegs [(i,i)] | i <- [0..posToCheck] ] 
        let arr = parMap rseq solveInitialBoard startStates 
        getShortestList arr []
